...
```{bash}
# conda create -n TumorTransition python=3.8
# conda activate TumorTransition
...
### 3.1.4 Define of tumor transition
...
Next, we start define the transition of putative tumor cluster, the processed plots and data were saved to OutDir:
```{r}
# TumorSTn <-
#   TransitionDefine(
#     TumorST = TumorST,
...
After defined the transition of tumor, we need to project the tumor spots, transition spots and other spots to spatial HE/IF staining plot. Finally, we generated and saved a transition defined Seurat object for future analysis.
```{r}
# TumorST <-
#   TransitionPlot(
#     TumorSTn = TumorSTn,
...
To better understand of cell composition of tumor transition as well as tumor and out tumor regions, we combined tumor ST data and tumor scRNAseq data and used deconvolution method to estimate the proportion of main cell types in scRNAseq data infiltrated in each spatial spot.
...
### 3.2.4 Visualization
After getting the deconvolution result, we need to visualize the result with pie plot and bar plot. Pie plot was used to project the main cell type infiltration of each spot to spatial. Bar plot was used to compare cell type infiltration of tumor, tumor transition and other out tumor spots.
```{r}
...
### 3.3.2 Differentially expressed genes of tumor transition
To reconstruct of spatial tumor microenvironment, we firstly find differentially expressed genes (DEG) in transition:
```{r}
# DiffGenes <- FindDiffGenes(TumorST = TumorST, assay = "Spatial")
```

Then, we visualized tumor transition DEG use volcano plot. If you want to visualize other types of ST data spots, you can change parameter Location to "Tumor" or "OutSpots".
```{r}
# DiffVolcanoplot(
#   DiffGenes = DiffGenes,
#   Location = "Trans",
...
After find the DEG of tumor transition, we explored the KEGG and GO pathways theses features enriched in and speculated the possibly function of tumor transition
```{r}
# transition_enrich <-
#   FeatureEnrichment(
#     DiffGenes = DiffGenes,
