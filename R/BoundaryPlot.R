...
BoundaryPlot <- function(TumorSTn = TumorSTn,
  ...
  Mal_ID <- position[Mal_barcode,]$spot.ids
  Bdy_ID <- position[Bdy_barcode,]$spot.ids

  Normal_Bdy_ID = unique(unlist(nbrs(df_j = df_j,MalSpotIDAdd = Mal_ID,SpotIDRaw = c(Bdy_ID,Mal_ID))))
  Normal_Bdy_barcode <- rownames(position[position$spot.ids %in% Normal_Bdy_ID,])

  nMal_barcode = rownames(TumorST@meta.data) %>% .[! .%in% c(Mal_barcode,Bdy_barcode,Normal_Bdy_barcode)]

  Barcode_Ann <- data.frame(barcode=c(Mal_barcode,Bdy_barcode,Normal_Bdy_barcode,nMal_barcode),
                            Location= c(rep("Mal",length(Mal_barcode)),
  ...
  TumorST_DefineColors <- data.frame(Location=c("Mal","Bdy","nMal"),
