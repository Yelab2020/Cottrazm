...
#'
#' Visualize transition define results of ST data
#'
#' @param TumorSTn A subset Seurat object of ST data with defined tumor, transition zone, and normal spots
...
#'
#' @return A Seurat object with defined tumor spots, transition zone spots, and out spots
#' @export
#'
#' @examples
#'
...
#' TumorLabel = c(1,2)
#' TumorSTn <- TransitionDefine(TumorST = TumorST, TumorLabel = TumorLabel, OutDir = OutDir, Sample = Sample)
#' TumorST <- TransitionPlot(TumorSTn = TumorSTn, TumorST = TumorST, OutDir = OutDir,Sample = Sample)
#'
TransitionPlot <- function(TumorSTn = TumorSTn,
                         TumorST = TumorST,
                         OutDir = OutDir,
                         Sample = Sample){
  ...
  pdf(paste(OutDir,Sample,"TransitionDefine.pdf",sep = ""),width = 7,height = 7)
  p <- SpatialDimPlot(TumorST,group.by = "Location",cols = TumorST_DefineColors$colors)
  print(p)
  dev.off()

  readr::write_rds(TumorST,paste(OutDir,Sample,"TransitionDefine.rds.gz",sep = ""),compress = "gz")
