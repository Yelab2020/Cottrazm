...
spot_proportion_initial <- function(enrich_matrix = enrich_matrix,
  ...
  for (i in 1:length(topic_sort)){
    ...
    if (malignant_cluster %in% ct_sig & tissue_cluster %in% ct_sig){ct_sig <- ct_sig[ct_sig != malignant_cluster]} #signature score can't tell malignant form epithelial
    ...
    if (length(ct_sig) == 0){
      ...
      if(length(ct_sig) ==1){
        ...
        if (ct_sig %in% ct_enrich){ct <- ct_enrich}else{ct <- c(ct_sig,ct_enrich[1])}


      }else{ct <- unique(c(ct_sig,ct_enrich))}
